...
**lsof -i  :22** - посмотреть кто использует порт.
...
**kernel. org** - информация о ядре, исходный код для ядра
**lsmod** - загруженные модули ядра

**modprobe "name module"** - загрузить модуль ядра

## Сети

**ss** , **netstat** - сокеты.

**https://www.iana.org/assignments/service-names-port-numbers** - Реестр имен служб и номеров портов транспортного протокола

**dhcp** - Red Hat

**ics-dhcp-server** - Debian

**netstate -nr** или **ip route show** - посмотреть существующие маршруты в системе.

**ethtool eth0** - характеристики и настройка интерфейса. до перезагрузки. 
...
**iperf** - проверка скорости сети.

**iptables**

**ip -br a** - публичный IP addres server

## Сеть

**resolvconf -u** - обновить записи DNS

**snmpwalk -v 2c -c public 10.37.144.8** - опросить принтеры по SNMP (1, 2c или 3)

## Домен

**getent passwd malov** - сведения о пользователе AD

**net ads info** - информация о домене

**net ads testjoin** - Проверить, действителен ли пароль учетной записи компьютера

**rolelst** - назначенные роли

**id malov** - посмотреть группы 

**realm leave --remove domain.ru -U admin** - удалить пк из домена

**control system-auth** - схема аутентификации

**control lightdm-greeter-hide-users show** - чтобы в окне входа отображался список доступных пользователей

**sss_cache -E** - удалить кэш sssd

**net ads keytab create** - сгенерировать системный keytab-файл

**hostnamectl** - информация о пк имя, ядро и тд
